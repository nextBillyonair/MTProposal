...
(EM) approach to word alignment can be transcribed as a neural network in a
...
the Alignment Error Rate (AER) is minimized. In a previous assignment, our task
was to implement IBM Model 1, based on the Expectation-Maximization (EM)
algorithm. Following IBM Model 1, we implemented a reparametrization of IBM
Model 2 that favors alignments along the diagonal. Finally, we implemented an
Alignment by Agreement Model that trained a French to English and English to
French model and combined the results to make better alignments.

Our proposal for this project is to replace the EM algorithm and
reparametrization of IBM Model 2 with a neural network architecture. In
addition, the model will incorporate Alignment by Agreement. We will use
PyTorch to build the model and loss functions. The key concept is that we can
create a loss function that the network will learn the optimal parameters to
...
% Expand discussion on VAE and similiarity to our proposal. MAybe include VAE
% diagram.
...
The work of Dyer et al. \cite{dyer2013simple} added an effective
reparameterization of IBM Model 2 to give an alignment distribution $a$. We
define for a French sentence \textbf{f} of length $n = |\textbf{f}|$, an
English sentence \textbf{e} of length $m=|\textbf{e}|$, with precision
parameter $\lambda$, the alignment distribution $a$ as follows:
...
This formulation gives us a position aware distribution that favors alignments
...
we then proceed to take the intersection of each model's alignments.
\begin{equation}
...
\end{equation}
This effectively gives us an alignment matrix that both models favor strongly,
i.e. agree on. From this intersection matrix, we can then use the
...
We will describe our model architecture to learn word alignments, along with
our description of a combined loss function that will encode alignment by
...
the pseudo M-Step evauation (loss) function.
...
In order to create our loss functions, we have to turn our alignment matrix
$\psi$ into a probability distribution. Let us define $\sigma(\vec{x})$ to be
...
While in VAEs you would sample from the probability distribution that is
returned by the neural network, our model's distribution is discrete. Therefore
we don't have to sample and instead can compute the loss in terms of all
possibilities. We can now describe our loss functions.


\subsubsection{Probability Maximization}

In the original formulation, we were maximizing our probabilities for word
...
This formula stems from the fact that we are assuming the alignment per target
word $t_j$ is independent of each other, hence a product over target words
$t_j$. Then we sum over all the alignment possibilities from the source words
$s_i$, thus marginalizing over the alignments.

However, we do not maximize in neural networks, and have to transform this
...
In order to calculate the KL divergence of our model, we need to define what
our distribution $P$ and $Q$ are. First, we will consider $P$ as our prior
distribution, henceforth called $a_0$. Our prior distribution $a_0$ is a
...
From Dyer's paper, parameter values were selected as $\lambda=4$ and $p_0=0.08$
...
\cite{dyer2013simple}. We can then write our KL Divergence, which is to be
minimized:
